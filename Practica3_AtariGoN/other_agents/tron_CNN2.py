...
# TODO: Queda pendiente para esta implementación, colocar las piedras de los agentes en un
#       canal de manera independiente. ¿A cuántos agentes debería truncarse? ¿Es conveniente esto?

class DQN(nn.Module):
    def __init__(self, input_channels, board_size, num_actions):
        super(DQN, self).__init__()
        self.conv1 = nn.Conv2d(in_channels=input_channels, out_channels=32, kernel_size=3, stride=1)
        self.conv2 = nn.Conv2d(in_channels=32, out_channels=64, kernel_size=3, stride=1)
        self.conv3 = nn.Conv2d(in_channels=64, out_channels=64, kernel_size=3, stride=1)
        self.conv4 = nn.Conv2d(in_channels=64, out_channels=128, kernel_size=3, stride=1)
        self.fc1 = nn.Linear(in_features=self._calculate_conv_output(input_channels, board_size), out_features=512)
        self.fc2 = nn.Linear(in_features=512, out_features=256)
        self.fc3 = nn.Linear(in_features=256, out_features=num_actions)

    def _calculate_conv_output(self, input_channels, board_size):
...
class TronGoshi(Goshi):
    ...
    policy_dqn = DQN(input_channels=2, board_size=goban_size, num_actions= goban_size**2)
    target_dqn = DQN(input_channels=2, board_size=goban_size, num_actions= goban_size**2)
    ...
    def optimize(self, mini_batch):
        ...
        for state, action, new_state, reward, terminated in mini_batch:
            old_state = torch.from_numpy(state).float()  # Convert state to float tensor
            old_new_state = torch.from_numpy(new_state).float()  # Convert new_state to float tensor
            
            state = old_state.unsqueeze(0).unsqueeze(0)
            new_state = old_new_state.unsqueeze(0).unsqueeze(0)
        ...
        torch.save(self.policy_dqn.state_dict(), "tronCNN.pt")
